...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        at_format = "At({}, {})".format
        cargo_format = "Cargo({})".format
        plane_format = "Plane({})".format
        airport_format = "Airport({})".format
        in_format = "In({}, {})".format
        ...
        def load_actions():
            ...
            for cargo, plane, airport in product(self.cargos, self.planes, self.airports):
                ...
                precond = [[expr(at_format(cargo, airport)), expr(at_format(plane, airport))] , []]

                effect = [[expr(in_format(cargo, plane))], [expr(at_format(cargo, airport))]]

                loads.append(Action(action, precond, effect))
        ...
        def unload_actions():
            ...
            for cargo, plane, airport in product(self.cargos, self.planes, self.airports):
                ...
                precond_pos = [expr(in_format(cargo, plane)), expr(at_format(plane, airport))]
                precond_neg = []

                effect_add = [expr(at_format(cargo, airport))]
                effect_rem = [expr(in_format(cargo, plane))]
